...
class SmartBlogTag extends Module {
        ...
        $this->displayName = Context::getContext()->getTranslator()->trans('Smart Blog Tag', array(), 'Modules.smartblogtag');
        $this->description = Context::getContext()->getTranslator()->trans('The Most Powerfull Presta shop Blog  Module\'s tag - by doradothemes', array(), 'Modules.smartblogtag');
        $this->confirmUninstall = Context::getContext()->getTranslator()->trans('Are you sure you want to delete your details ?', array(), 'Modules.smartblogtag');
        }
                ...
                '._DB_PREFIX_.'smart_blog_tag t ON p.id_tag= t.id_tag where t.id_lang = '.(int)$id_lang.' LIMIT '.$limit;
                          ...
                          if(count($tags) > 0){
                            foreach ($tags as $key => $tag) {
                              $tag['alias'] = str_replace(" ","-",$tag['name']);
                              $itemTags[] = $tag;
                            }
                            $tags = $itemTags;
                          }
         ...
         public function hookdisplaySmartBlogRight($params)
            {
                ...
                if(Tools::isSubmit('save'.$this->name))
                {
                    ...
                    $html = $this->displayConfirmation(Context::getContext()->getTranslator()->trans('The settings have been updated successfully.', array(), 'Modules.smartblogtag'));
     ...
     public function SettingForm() {
        ...
        $this->fields_form[0]['form'] = array(
          'legend' => array(
          'title' => Context::getContext()->getTranslator()->trans('General Setting', array(), 'Modules.smartblogtag'),
            ),
            'input' => array(
                array(
                    'type' => 'text',
                    'label' => Context::getContext()->getTranslator()->trans('Number of Tag Show', array(), 'Modules.smartblogtag'),
            ...
            'submit' => array(
                'title' => Context::getContext()->getTranslator()->trans('Save', array(), 'Modules.smartblogtag'),
                'class' => 'button'
            )
        );
        ...
        $helper->toolbar_btn = array(
            'save' =>
            array(
                'desc' => Context::getContext()->getTranslator()->trans('Save', array(), 'Modules.smartblogtag'),
