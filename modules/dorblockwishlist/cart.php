...
{
	if ($id_wishlist && DorWishList::exists($id_wishlist, $context->customer->id) === true)
		$context->cookie->id_wishlist = (int)$id_wishlist;

	if ((int)$context->cookie->id_wishlist > 0 && !DorWishList::exists($context->cookie->id_wishlist, $context->customer->id))
	...
	if (($add || $delete) && empty($id_product) === false)
	{
		if (!isset($context->cookie->id_wishlist) || $context->cookie->id_wishlist == '')
		{
			...
			$mod_wishlist = new DorBlockWishList();
		...
		if ($add && $quantity)
			DorWishList::addProduct($context->cookie->id_wishlist, $context->customer->id, $id_product, $id_product_attribute, $quantity);
		else if ($delete)
			DorWishList::removeProduct($context->cookie->id_wishlist, $context->customer->id, $id_product, $id_product_attribute);
	}
	$context->smarty->assign('products', DorWishList::getProductByIdCustomer($context->cookie->id_wishlist, $context->customer->id, $context->language->id, null, true));

	if (Tools::file_exists_cache(_PS_THEME_DIR_.'modules/dorblockwishlist/blockwishlist-ajax.tpl'))
		$context->smarty->display(_PS_THEME_DIR_.'modules/dorblockwishlist/blockwishlist-ajax.tpl');
