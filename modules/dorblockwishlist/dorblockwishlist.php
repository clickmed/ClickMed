...
include_once(dirname(__FILE__).'/DorWishList.php');

class DorBlockWishList extends Module
{
	...
	{
		$this->name = 'dorblockwishlist';
		$this->tab = 'front_office_features';
		$this->version = '2.0.0';
		$this->author = 'Dorado Themes';
		$this->need_instance = 0;

		$this->controllers = array('dorwishlist', 'view');

		$this->bootstrap = true;
		parent::__construct();

		$this->displayName = $this->l('Dor Wishlist block');
	...
	private function deleteTables()
	{
		return Db::getInstance()->execute(
			...
			`'._DB_PREFIX_.'dorwishlist`,
			`'._DB_PREFIX_.'dorwishlist_email`,
			`'._DB_PREFIX_.'dorwishlist_product`,
			`'._DB_PREFIX_.'dorwishlist_product_cart`'
		);
	}
	...
	public function getContent()
	{
		...
		if (Tools::getValue('id_customer') && Tools::getValue('id_dorwishlist'))
			$this->html .= $this->renderList((int)Tools::getValue('id_dorwishlist'));
	...
	public function hookDisplayProductListFunctionalButtons($params)
	{
		...
		if ($this->context->customer->isLogged())
			$this->smarty->assign('wishlists', DorWishList::getByIdCustomer($this->context->customer->id));

		$this->smarty->assign('product', $params['product']);
		return $this->display(__FILE__, 'blockwishlist_button.tpl');
	}
	public function hookDorwishlist($params){
		return $this->display(__FILE__, 'blockwishlist_button_item.tpl');
	}
	public function hookTop($params)
	{
		if ($this->context->customer->isLogged())
		{
			$wishlists = DorWishList::getByIdCustomer($this->context->customer->id);
			if (empty($this->context->cookie->id_dorwishlist) === true ||
				DorWishList::exists($this->context->cookie->id_dorwishlist, $this->context->customer->id) === false)
			{
				if (!count($wishlists))
					$id_dorwishlist = false;
				else
				{
					$id_dorwishlist = (int)$wishlists[0]['id_dorwishlist'];
					$this->context->cookie->id_dorwishlist = (int)$id_dorwishlist;
				}
			}
			else
				$id_dorwishlist = $this->context->cookie->id_dorwishlist;

			$this->smarty->assign(
				array(
					'id_dorwishlist' => $id_dorwishlist,
					'isLogged' => true,
					'wishlist_products' => ($id_dorwishlist == false ? false : DorWishList::getProductByIdCustomer($id_dorwishlist,
	...
	{
		$this->context->controller->addCSS(($this->_path).'css/dorwishlist.css', 'all');
	...
	private function _displayProducts($id_dorwishlist)
	{
		include_once(dirname(__FILE__).'/DorWishList.php');

		$wishlist = new DorWishList($id_dorwishlist);
		$products = DorWishList::getProductByIdCustomer($id_dorwishlist, $wishlist->id_customer, $this->context->language->id);
	...
	public function hookAdminCustomers($params)
	{
		...
		$wishlists = DorWishList::getByIdCustomer((int)$customer->id);
		if (!count($wishlists))
		...
		else
		{
			...
			if (!$id_dorwishlist)
				$id_dorwishlist = $wishlists[0]['id_dorwishlist'];

			$this->html .= '<span>'.$this->l('Wishlist').': </span> <select name="id_dorwishlist" onchange="$(\'#listing\').submit();">';

			if (is_array($wishlists))
				foreach ($wishlists as $wishlist)
				{
					$this->html .= '<option value="'.(int)$wishlist['id_dorwishlist'].'"';
					if ($wishlist['id_dorwishlist'] == $id_dorwishlist)
					{
			...
			$this->_displayProducts((int)$id_dorwishlist);
	...
	*/
	...
	{
		return "<script>
			$(document).ready(function () { $('#id_customer, #id_dorwishlist').change( function () { $('#module_form').submit();}); });
		</script>";
	}
	...
	{
		$customers = array();
        	foreach (DorWishList::getCustomers() as $c)
        	{
		...
		{
			$wishlists = DorWishList::getByIdCustomer($id_customer);
	...
	public function renderList($id_dorwishlist)
	{
		$wishlist = new DorWishList($id_dorwishlist);
		$products = DorWishList::getProductByIdCustomer($id_dorwishlist, $wishlist->id_customer, $this->context->language->id);
