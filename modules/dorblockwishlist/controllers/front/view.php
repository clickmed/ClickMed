...
class DorBlockWishListViewModuleFrontController extends ModuleFrontController
{
	...
	public function __construct()
	{
		...
		include_once($this->module->getLocalPath().'dorblockwishlist.php');
	}
	...
	public function initContent()
	{
		...
		if ($token)
		{
			$wishlist = DorWishList::getByToken($token);

			DorWishList::refreshWishList($wishlist['id_dorwishlist']);
			$products = DorWishList::getProductByIdCustomer((int)$wishlist['id_dorwishlist'], (int)$wishlist['id_customer'], $this->context->language->id, null, true);
			...
			DorWishList::incCounter((int)$wishlist['id_dorwishlist']);
			$ajax = Configuration::get('PS_BLOCK_CART_AJAX');

			$wishlists = DorWishList::getByIdCustomer((int)$wishlist['id_customer']);

			foreach ($wishlists as $key => $item)
			{
				if ($item['id_dorwishlist'] == $wishlist['id_dorwishlist'])
				{
					unset($wishlists[$key]);
					break;
				}
