...
<div id="dor-tabsidebar-product-category3" class="sidebar3">
	<div class="dor-tabsidebar-product-category-wrapper">
		<ul role="tablist" class="nav nav-tabs dorTabAjaxSidebar" id="dorTabAjaxSidebar" data-ajaxurl="{if isset($urls.force_ssl) && $urls.force_ssl}{$urls.base_url_ssl}{else}{$urls.base_url}{/if}modules/dor_ajaxtabsidebar_product3/productcategory-ajax.php">
			{$j=0}
		...
		<div class="tab-content" id="dorTabSidebarProductCategoryContent3">
		{if $listTabsProduct}
			{$k=0}
			{foreach from=$listTabsProduct item=productTab name=tabCatePro}
			<div aria-labelledby="cate-tab-data-{$productTab.id}-tab" id="cate-tab-data-{$productTab.id}" class="tab-pane fade {if $productTab.id==$tabID.id} active {/if} in">
				...
				<div class="view-all">
					<a class="btn btn-scale btn-go-cate" href="{$urls.pages.index}{$productTab.url}"><span><span class="fa fa-long-arrow-right">&nbsp;</span></span>{l s='View All' mod="dor_ajaxtabsidebar_product3"}</a>
				</div>
			</div>
			{$k= $k+1}
			{/foreach}
		{/if}
			...
			<div aria-labelledby="cate-tab-data-{$cate.id}-tab" id="cate-tab-data-{$cate.id}" class="tab-pane fade {if $cate.id==$tabID.id} active {/if} in">
				...
				<div class="view-all">
					<a class="btn btn-scale btn-go-cate" href="{$urls.pages.index}"><span><span class="fa fa-long-arrow-right">&nbsp;</span></span>{l s='View All' mod="dor_ajaxtabsidebar_product3"}</a>
				</div>
			</div>
			{$key= $key+1}
			{/foreach}	
