...
class DorProductReviews extends Module
{
	...
	public function __construct()
	{
		...
		$this->displayName = $this->l('Dor Product Review Comments');
	...
	public function install($keep = true)
	{
		...
		if (parent::install() == false ||
			!$this->registerHook('dorProductReviews') ||
			!$this->registerHook('dorProductDetailReviews') ||
	...
	public function uninstall($keep = true)
	{
		if (!parent::uninstall() || ($keep && !$this->deleteTables()) ||
			...
			!$this->unregisterHook('dorProductReviews') ||
			!$this->unregisterHook('dorProductDetailReviews') ||
	...
	public function deleteTables()
	{
		return Db::getInstance()->execute('
			...
			`'._DB_PREFIX_.'dorproduct_comment`,
			`'._DB_PREFIX_.'dorproduct_comment_criterion`,
			`'._DB_PREFIX_.'dorproduct_comment_criterion_product`,
			`'._DB_PREFIX_.'dorproduct_comment_criterion_lang`,
			`'._DB_PREFIX_.'dorproduct_comment_criterion_category`,
			`'._DB_PREFIX_.'dorproduct_comment_grade`,
			`'._DB_PREFIX_.'dorproduct_comment_usefulness`,
			`'._DB_PREFIX_.'dorproduct_comment_report`');
	}
	...
	protected function _postProcess()
	{
		...
		{
			$id_product_comment = (int)Tools::getValue('id_dorproduct_comment');
			$comment = new DorProductReviews($id_product_comment);
			$comment->validate();
			ProductComment::deleteReports($id_product_comment);
		}
		elseif (Tools::isSubmit('deletedorproductreviews'))
		{
			$id_product_comment = (int)Tools::getValue('id_dorproduct_comment');
		...
		{
			$criterion = new DorProductCommentCriterion((int)Tools::getValue('id_dorproduct_comment_criterion'));
			$criterion->id_dorproduct_comment_criterion_type = Tools::getValue('id_dorproduct_comment_criterion_type');
			...
			if ($criterion->id_dorproduct_comment_criterion_type == 2)
			{
			...
			else if ($criterion->id_dorproduct_comment_criterion_type == 3)
			{
		...
		elseif (Tools::isSubmit('deletedorproductreviewscriterion'))
		{
			$productCommentCriterion = new DorProductCommentCriterion((int)Tools::getValue('id_dorproduct_comment_criterion'));
		...
		elseif (Tools::isSubmit('statusdorproductreviewscriterion'))
		{
			$criterion = new DorProductCommentCriterion((int)Tools::getValue('id_dorproduct_comment_criterion'));
			if ($criterion->id)
			{
	...
	public function getContent()
	{
		...
		include_once(dirname(__FILE__).'/DorProductCommentCriterion.php');

		$this->_html  = '';
		if (Tools::isSubmit('updatedorproductreviewscriterion'))
			$this->_html .= $this->renderCriterionForm((int)Tools::getValue('id_dorproduct_comment_criterion'));
		else
		{
			$this->_postProcess();
		...
		$this->_setBaseUrl();
		$this->_productCommentsCriterionTypes = DorProductCommentCriterion::getTypes();
	...
	public function renderModerateLists()
	{
		...
		if (Configuration::get('PRODUCT_COMMENTS_MODERATE'))
		{
			...
			$helper->identifier = 'id_dorproduct_comment';
	...
	public function renderCriterionList()
	{
		...
		$criterions = DorProductCommentCriterion::getCriterions($this->context->language->id, false, false);

		$fields_list = array(
			'id_dorproduct_comment_criterion' => array(
				'title' => $this->l('ID'),
				'type' => 'text',
			),
			'name' => array(
		...
		$helper->toolbar_btn['new'] = array(
			'href' => $this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'&module_name='.$this->name.'&updatedorproductreviewscriterion',
			'desc' => $this->l('Add New Criterion', null, null, false)
		);
		$helper->module = $this;
		$helper->identifier = 'id_dorproduct_comment_criterion';
	...
	{
		$criterion = new DorProductCommentCriterion($id);

		return array(
					'name' => $criterion->name,
					'id_dorproduct_comment_criterion_type' => $criterion->id_dorproduct_comment_criterion_type,
					'active' => $criterion->active,
					'id_dorproduct_comment_criterion' => $criterion->id,
				);
	}
	...
	{
		$types = DorProductCommentCriterion::getTypes();
		$query =array();
		...
		$criterion = new DorProductCommentCriterion((int)$id_criterion);
		...
		$fields_form_1 = array(
			'form' => array(
				...
				'input' => array(
					array(
						'type' => 'hidden',
						'name' => 'id_dorproduct_comment_criterion',
					),
					array(
						'type' => 'text',
						'lang' => true,
					...
					array(
						'type' => 'select',
						'name' => 'id_dorproduct_comment_criterion_type',
	...
	public function hookProductTab($params)
	{
		...
		require_once(dirname(__FILE__).'/ProductComment.php');
		require_once(dirname(__FILE__).'/DorProductCommentCriterion.php');
		...
		$this->context->smarty->assign(array(
											...
											'criterions' => DorProductCommentCriterion::getByProduct((int)(Tools::getValue('id_product')), $this->context->language->id),
	...
	public function hookProductTabContent($params)
	{
		...
		$this->context->smarty->assign(array(
			...
			'mediumSize' => Image::getSize(ImageType::getFormatedName('medium')),
			'nbComments' => (int)ProductComment::getCommentNumber((int)Tools::getValue('id_product')),
			'productcomments_controller_url' => $this->context->link->getModuleLink('dorproductreviews'),
	...
	public function hookDorProductReviews($params)
	{
		$this->context->controller->addJS($this->_path.'js/jquery.rating.pack.js');
		$this->context->controller->addJS($this->_path.'js/jquery.textareaCounter.plugin.js');
		$this->context->controller->addJS($this->_path.'js/productcomments.js');
	...
	public function hookExtraProductComparison($params)
	{
		...
		foreach ($params['list_ids_product'] as $id_product)
		{
			...
			$criterions = DorProductCommentCriterion::getByProduct($id_product, $this->context->language->id);
			$grade_total = 0;
			if (count($grades) > 0)
			{
				foreach ($criterions as $criterion)
				{
					if (isset($grades[$criterion['id_dorproduct_comment_criterion']]))
					{
						$list_product_grades[$criterion['id_dorproduct_comment_criterion']][$id_product] = $grades[$criterion['id_dorproduct_comment_criterion']];
						$grade_total += (float)($grades[$criterion['id_dorproduct_comment_criterion']]);
					}
					else
						$list_product_grades[$criterion['id_dorproduct_comment_criterion']][$id_product] = 0;

					if (!array_key_exists($criterion['id_dorproduct_comment_criterion'], $list_grades))
						$list_grades[$criterion['id_dorproduct_comment_criterion']] = $criterion['name'];
				}
	...
	public function initCategoriesAssociation($id_root = null, $id_criterion = 0)
	{
		...
		{
			$pdc_object = new DorProductCommentCriterion($id_criterion);
