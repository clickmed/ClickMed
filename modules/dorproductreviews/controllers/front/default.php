...
// Include Module
include_once(dirname(__FILE__).'/../../dorproductreviews.php');
// Include Models
include_once(dirname(__FILE__).'/../../ProductComment.php');
include_once(dirname(__FILE__).'/../../DorProductCommentCriterion.php');

class DorProductReviewsDefaultModuleFrontController extends ModuleFrontController
{
	...
	{
		$module_instance = new DorProductReviews();
		...
		if (!count($errors))
		{
			...
			if (!$customer_comment || ($customer_comment && (strtotime($customer_comment['date_add']) + (int)Configuration::get('PRODUCT_COMMENTS_MINIMAL_TIME')) < time()))
			{
				...
				$comment->website = Tools::getValue('website');
				...
				$grade_sum = 0;
				foreach(Tools::getValue('criterion') as $id_dorproduct_comment_criterion => $grade)
				{
					$grade_sum += $grade;
					$dorproduct_comment_criterion = new DorProductCommentCriterion($id_dorproduct_comment_criterion);
					if ($dorproduct_comment_criterion->id)
						$dorproduct_comment_criterion->addGrade($comment->id, $grade);
				}
	...
	{
		if (!Tools::isSubmit('id_dorproduct_comment'))
			die('0');

		if (ProductComment::isAlreadyReport(Tools::getValue('id_dorproduct_comment'), $this->context->cookie->id_customer))
			die('0');

		if (ProductComment::reportComment((int)Tools::getValue('id_dorproduct_comment'), $this->context->cookie->id_customer))
			die('1');
	...
	{
		if (!Tools::isSubmit('id_dorproduct_comment') || !Tools::isSubmit('value'))
			die('0');

		if (ProductComment::isAlreadyUsefulness(Tools::getValue('id_dorproduct_comment'), $this->context->cookie->id_customer))
			die('0');

		if (ProductComment::setCommentUsefulness((int)Tools::getValue('id_dorproduct_comment'), (bool)Tools::getValue('value'), $this->context->cookie->id_customer))
			die('1');
