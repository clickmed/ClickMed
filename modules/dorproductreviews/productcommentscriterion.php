	...
	isset($_GET['id_product']) === true)
{
	...
	echo '<select name="id_dorproduct_comment_criterion[]" id="id_dorproduct_comment_criterion" multiple="true" style="height:100px;width:360px;">';
	foreach ($criterions as $criterion)
	{
		echo '<option value="'.(int)($criterion['id_dorproduct_comment_criterion']).'"';
		if (isset($selects) === true && sizeof($selects))
		{
			foreach ($selects as $select)
				if ($select['id_dorproduct_comment_criterion'] == $criterion['id_dorproduct_comment_criterion'])
