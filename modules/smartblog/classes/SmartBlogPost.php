...
class SmartBlogPost extends ObjectModel
{
    ...
    public static function getRecentPosts($id_lang = null){
		...
		$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT  p.id_smart_blog_post,p.created,pl.meta_title,pl.link_rewrite, sbcl.`meta_title` category_name, sbcl.`link_rewrite` category_link_rewrite FROM '._DB_PREFIX_.'smart_blog_post p INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_lang pl ON p.id_smart_blog_post=pl.id_smart_blog_post INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_shop ps ON pl.id_smart_blog_post = ps.id_smart_blog_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_lang sbcl ON (p.id_category = sbcl.id_smart_blog_category AND sbcl.id_lang='.(int) $id_lang.')
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_shop sbcs ON (p.id_category = sbcs.id_smart_blog_category AND sbcs.id_shop='.(int) Context::getContext()->shop->id.')
    ...
    public static function GetPostMetaByPost($id_post,$id_lang = null){
        ...
        public static function GetPostLatestHome($limit)
            ...
            $result = array();
            $sql = 'SELECT p.*, pl.*, sbcl.`meta_title` category_name, sbcl.`link_rewrite` category_link_rewrite FROM '._DB_PREFIX_.'smart_blog_post p INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_lang pl ON p.id_smart_blog_post=pl.id_smart_blog_post INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_shop ps ON pl.id_smart_blog_post = ps.id_smart_blog_post AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_lang sbcl ON (p.id_category = sbcl.id_smart_blog_category AND sbcl.id_lang='.(int) $id_lang.')
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_shop sbcs ON (p.id_category = sbcs.id_smart_blog_category AND sbcs.id_shop='.(int) Context::getContext()->shop->id.')
            ...
            if(empty($posts)){
                $sql2 = 'SELECT p.*, pl.*, sbcl.`meta_title` category_name, sbcl.`link_rewrite` category_link_rewrite FROM '._DB_PREFIX_.'smart_blog_post p INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_lang pl ON p.id_smart_blog_post=pl.id_smart_blog_post INNER JOIN 
                '._DB_PREFIX_.'smart_blog_post_shop ps ON pl.id_smart_blog_post = ps.id_smart_blog_post  AND ps.id_shop = '.(int) Context::getContext()->shop->id.'
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_lang sbcl ON (p.id_category = sbcl.id_smart_blog_category AND sbcl.id_lang='.(int) $id_lang.')
                LEFT JOIN '._DB_PREFIX_.'smart_blog_category_shop sbcs ON (p.id_category = sbcs.id_smart_blog_category AND sbcs.id_shop='.(int) Context::getContext()->shop->id.')
            ...
            foreach($posts as $post){
                ...
                $result[$i]['is_featured'] = $post['is_featured'];
                $result[$i]['link_rewrite'] = $post['link_rewrite'];
                $result[$i]['category_name'] = $post['category_name'];
                $result[$i]['category_link_rewrite'] = $post['category_link_rewrite'];
                $employee = new  Employee( $post['id_author']);
                $result[$i]['lastname'] = $employee->lastname;
                $result[$i]['firstname'] = $employee->firstname;
                if (file_exists(_PS_MODULE_DIR_.'smartblog/images/' . $post['id_smart_blog_post'] . '.jpg'))
