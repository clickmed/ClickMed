...
class  smartblogDetailsModuleFrontController extends smartblogModuleFrontController
{
	...
	public function initContent()
	{
           ...
           $title_category = $BlogCategory->getNameCategory($id_cate);
            ...
            $pathImg = "smartblog/images/".$post['post_img'];
            $thumbWidth = Configuration::get('blogThumbDetailWidth');
            $thumbHeight = Configuration::get('blogThumbDetailHeight');
            $thumbWidth = $thumbWidth != ""?$thumbWidth:740;
            $thumbHeight = $thumbHeight != ""?$thumbHeight:366;
            $sizeThumb = $thumbWidth."x".$thumbHeight;
            $images = DorImageBase::renderThumb($pathImg,$thumbWidth,$thumbHeight);
            $post['thumb_image'] = $images;
            
	   SmartBlogPost::postViewed($id_post);
            $page_name = 'dorSmartBlogsDetail';
            $dorBlogsDetailStyle  = Tools::getValue('dorBlogsDetailStyle',Configuration::get('dorBlogsDetailStyle'));
           if(isset($dorBlogsDetailStyle) && $dorBlogsDetailStyle == 2){
                $tmpDetail = 'smartblog/posts-v2.tpl';
                $page_name = 'dorSmartBlogsDetailV2';
            }else{
                $tmpDetail = 'smartblog/posts.tpl';
            }
