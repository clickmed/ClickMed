/******************************************************
 * @package Dor Verticalmenu module for Prestashop 1.6.x
 * @version 1.0.0
...
var dorVerticalmenuForm = {
	options:{},
	modules_dir:'modules',
	init:function(){
		dorVerticalmenuForm.accordionInit();
		dorVerticalmenuForm.addToMenu();
		dorVerticalmenuForm.menuPosition();
		dorVerticalmenuForm.submenuSetting();
		dorVerticalmenuForm.editMenu();
		dorVerticalmenuForm.deleteMenu();
		dorVerticalmenuForm.addWidget();
		dorVerticalmenuForm.editWidgetForm();
		dorVerticalmenuForm.deleteWidget();
		//DorVerticalmenuForm.ClearStorage();
	},
	...
	submenuInit:function(data){
		if (typeof data != 'undefined' && data != '') {
			...
			$.each( data.rows, function( index, row ) {
				...
				$('.verticalmenu-setting-content').append( rowobj );
				dorVerticalmenuForm.changeActiveRow( rowobj );
				...
				$.each( row.cols, function( index_col, col ) {
					...
					dorVerticalmenuForm.changeActiveColumn( column );
				});
			});
	...
	submenuSetting:function(){
		$('body').on('click', '.menu-item-bar .sub-setting', function(){
			...
			dorVerticalmenuForm.addClassSaveButton('save-submenu');
		...
		$('body').on('click', '#submenu-form .add-row', function(){
			...
			$('.verticalmenu-setting-content').append( row );
			dorVerticalmenuForm.changeActiveRow( row );
		});
		// change active row
		$('body').on('click', '#submenu-form .dor-row', function(){
			dorVerticalmenuForm.changeActiveRow( $(this) );
			return false;
		});
		// delete row
		$('body').on('click', '#submenu-form .delete-row', function(){
			var row = dorVerticalmenuForm.getCurrentActiveRow();
			if (row == false) {
		...
		$('body').on('click', '#submenu-form .dor-column', function(){
			dorVerticalmenuForm.changeActiveColumn( $(this) );
			
			return false;
		});
		// delete column
		$('body').on('click', '#submenu-form .delete-column', function(){
			var column = dorVerticalmenuForm.getCurrentActiveColumn();
			if (column == false) {
		...
		$('body').on('click', '#save-button.save-submenu', function(){
			if ($('#submenu-form .verticalmenu-setting-content .dor-row').length > 0) {
				var data = new Object();
				data.rows = new Array();
				$('#submenu-form .verticalmenu-setting-content .dor-row').each(function(){
					var row =  new Object();
					row.cols = new Array();
 	 	 	...
 	 	 	$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data : serialized + '&' + params + '&action=updateSubMenu&secure_key=' + secure_key
			}).done( function ( reponse ) {
	...
	changeActiveColumn:function(current){
		...
		dorVerticalmenuForm.changeActiveRow( current_row );
		current.addClass('active');
		$('#submenu-form .column_width').val( current.data('col_width') ).change();

		$('.verticalmenu-setting-header .columncls').show();
	},
	editMenu:function(){
		$('body').on('click', '#menu-form-list .menu-item-bar .quickedit', function(){
			var id_menu = $(this).data('value');
			$('#menuModalLabel').text( editmenu_title );
			dorVerticalmenuForm.addClassSaveButton('save-button');
			$.ajax({
	            url: adminajaxurl,
	            type: 'POST',
	            dataType: 'html',
	            data:  'action=editMenu&id_menu=' + id_menu + '&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	$("#menuModal .modal-body").html( reponse );
	        	$('#menuModal').modal();
	        });
		...
		$('body').on('click', '#save-button.save-button', function(){
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data : serialized + '&action=updateMenu&secure_key=' + secure_key
			}).done( function ( reponse ) {
				if (reponse.status == 'ok') {
		            $('.menu-form-list-wrapper').html(reponse.html);
		            dorVerticalmenuForm.displayMsg('success', reponse.msg);
		            $('#menuModal').modal('toggle');
		            dorVerticalmenuForm.addClassSaveButton('');

		            dorVerticalmenuForm.menuPosition();
		        } else {
		        	dorVerticalmenuForm.displayMsg('error', reponse.msg);
		        }
		        $('#save-button').removeClass('btn-warning').addClass('btn-primary').attr("disabled", false);
			});
		});
		...
		$('body').on('click', '#menu-form-list .menu-item-bar .quickdel', function(){
			...
			if (verify == true) {
				$.ajax({
		            ...
		            type: 'POST',
		            dataType: 'json',
		            data:  'action=deleteMenu&id_menu=' + id_menu + '&secure_key=' + secure_key
		        }).done( function ( reponse ) {
		        	if (reponse.status == 'ok') {
			            $('.menu-form-list-wrapper').html(reponse.html);
			            dorVerticalmenuForm.displayMsg('success', reponse.msg);
	...
	menuPosition:function(){
		...
		$('.save-menu-position').click(function(){
			...
			$.ajax({
				...
				type: 'POST',
				dataType: 'json',
				data : serialized + '&action=updatePosition&secure_key=' + secure_key
			}).done( function (reponse) {
				dorVerticalmenuForm.displayMsg(reponse.status, reponse.msg);
	...
	addToMenu:function(){
		...
		$('.add-to-menu').click(function(){
			...
			$.ajax({
	            ...
	            type: 'POST',
	            dataType: 'json',
	            data:  'action=addMenu&values=' + values + '&type=' + type + '&custom_name=' + custom_name + '&custom_link=' + custom_link + '&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	if (reponse.status == 'ok') {
		            $('.menu-form-list-wrapper').html(reponse.html);
		            dorVerticalmenuForm.displayMsg('success', reponse.msg);
	...
	addWidget:function(){
		...
		$('button.add-widget').click(function(){
			...
			$.ajax({
	            ...
	            type: 'POST',
	            dataType: 'html',
	            data: 'action=listWidget&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	$("#menuModal .modal-body").html( reponse );
	        	$('#menuModal').modal();
	        });
		...
		$('body').on('click', '#menuModal .widget-item', function(){
			...
			dorVerticalmenuForm.addClassSaveButton('save-widget');

			$.ajax({
	            url: adminajaxurl,
	            type: 'POST',
	            dataType: 'html',
	            data: 'action=widgetForm&widget_type=' + widget_type + '&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	$("#menuModal .modal-body").html( reponse );
	        });
		});
		...
		$('body').on('click', '#save-button.save-widget', function(){
			...
			$.ajax({
	            ...
	            type: 'POST',
	            dataType: 'json',
	            data: serialized + '&action=addWidget&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	if (reponse.status == 'ok') {
		            $('.widget-list-items').html(reponse.html);
		            dorVerticalmenuForm.displayWidgetMsg('success', reponse.msg);
		            $('#menuModal').modal('toggle');
		            dorVerticalmenuForm.addClassSaveButton('');
		        } else {
		        	dorVerticalmenuForm.displayWidgetMsg('error', reponse.msg);
		        }
	        });
		});
	},
	editWidgetForm:function(){
		$('body').on('click', '.widget-list-items .menu-item-bar .quickedit', function(){
			...
			$.ajax({
	            ...
	            type: 'POST',
	            dataType: 'html',
	            data:  'action=widgetForm&id=' + id + '&secure_key=' + secure_key
	        }).done(function(reponse) {
	        	$("#menuModal .modal-body").html( reponse );
	        	$('#menuModal').modal();
	        });
		...
		$('body').on('click', '.widget-list-items .menu-item-bar .quickdel', function(){
			...
			if (verify == true) {
				$.ajax({
		            ...
		            type: 'POST',
		            dataType: 'json',
		            data:  'action=deleteWidget&id=' + id + '&secure_key=' + secure_key
		        }).done( function ( reponse ) {
		        	if (reponse.status == 'ok') {
			            $('.widget-list-items').html(reponse.html);
			            dorVerticalmenuForm.displayWidgetMsg('success', reponse.msg);
