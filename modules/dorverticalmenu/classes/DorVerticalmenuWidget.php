...
class DorVerticalmenuWidget extends ObjectModel {

	public $id_dorverticalmenu_widget;
	public $name;
	public $type;
	public $params;
	public $wkey;
	...
	public $mod_name = 'dorverticalmenu';
	...
	public static $definition = array(
		'table' => 'dorverticalmenu_widget',
		'primary' => 'id_dorverticalmenu_widget',
		'fields' => array(
	...
	public function __construct($id = null, $id_lang = null, $id_shop = null)
	{
		...
		$this->mscript = new DorVerticalmenuMcrypt();
	}
		...
		if (!$this->engines)
		{
			...
			foreach ($wds as $w)
			{
				...
				if ($last != 'index.php')
				{
					...
					$class = 'DorVerticalmenuWidget'.Tools::ucfirst($f);
	 ...
	 */
	public static function getWidgets($idShop = 0)
	{
		$idShop = $idShop==0?(int)Context::getContext()->shop->id:$idShop;
		$sql = ' SELECT * FROM '._DB_PREFIX_.'dorverticalmenu_widget WHERE `id_shop` = '.(int)$idShop;
		return Db::getInstance()->executeS($sql);
	}

	public function deleteItem($id)
	{
		$sql = ' DELETE FROM '._DB_PREFIX_.'dorverticalmenu_widget WHERE id_dorverticalmenu_widget='.(int)$id;
	...
	public function getWidetById($id)
	{
		...
		$sql = ' SELECT * FROM '._DB_PREFIX_.'dorverticalmenu_widget WHERE id_dorverticalmenu_widget='.(int)$id;
		...
		if ($row)
		{
			...
			$output['id'] = $output['id_dorverticalmenu_widget'];
		}
		return $output;
	}
	...
	public function getWidetByKey($wkey)
	{
		...
		$sql = ' SELECT * FROM '._DB_PREFIX_.'dorverticalmenu_widget WHERE 1 AND wkey=\''.pSQL($wkey).'\'';
	...
	public function loadWidgets()
	{
		if (empty($this->widgets))
		{
			...
			{
				$widget['id'] = $widget['id_dorverticalmenu_widget'];
	...
	public static function renderAdminListWidgets()
    {
        ...
        if ($widgets && count($widgets)) {
            $output = '<ol>';
            foreach ($widgets as &$widget) {
                $output .= '<li id="list_'.$widget['id_dorverticalmenu_widget'].'">
                    <div class="menu-item-bar">
                        <span class="item-title">
                            <span class="menu-item-title">'.($widget['name'] ? $widget['name'] : '').' (ID:'.$widget['id_dorverticalmenu_widget'].')</span> 
                        </span>
                        <span class="settings pull-right">
                        	<span class="type">'.$widget['type'].'</span>
                            <span class="quickedit" data-value="'.$widget['id_dorverticalmenu_widget'].'" title="Edit">
                                <i class="icon-pencil"></i></span>
                            <span class="quickdel" data-value="'.$widget['id_dorverticalmenu_widget'].'" title="Delete">
