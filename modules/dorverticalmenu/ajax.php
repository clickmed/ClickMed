...
include_once('dorverticalmenu.php');

$obj_menu = new DorVerticalMenu();
...
if (Tools::getValue('action') == 'addMenu')
{
	...
	if ($type == 'product') {
		foreach ($values as $id_content) {
			if ( !empty($id_content) && Validate::isInt($id_content) ) {
				$obj = new ObjectDorVerticalmenu();

				$obj->id_parent = 1;
				$obj->value = (int)$id_content;
				$obj->type = (string)$type;
				$obj->active = 1;
				$obj->name = DorVerticalmenuHelper::getLangValueByType($id_content, $type);
	...
	foreach ($child as $id_parent => $menus)
	{
		...
		foreach ($menus as $id_dorverticalmenu)
		{
			$res &= Db::getInstance()->execute('
                UPDATE `'._DB_PREFIX_.'dorverticalmenu` SET `position` = '.(int)$i.', id_parent = '.(int)$id_parent.' 
                WHERE `id_dorverticalmenu` = '.(int)$id_dorverticalmenu
			);
			$i++;
		}
	}
...
} elseif (Tools::getValue('action') == 'updateMenu' && Tools::getValue('id_dorverticalmenu') ) {
	$obj = new ObjectDorVerticalmenu((int)Tools::getValue('id_dorverticalmenu'));
	$result = false;
	...
	$obj = new ObjectDorVerticalmenu((int)Tools::getValue('id_menu'));
	$result = false;
	if (Validate::isLoadedObject($obj)) {
		if ($obj->delete()) {
			$result = true;
	...
	if (Tools::getValue('id_dorverticalmenu_widget')) {
		$obj = new DorVerticalmenuWidget((int)Tools::getValue('id_dorverticalmenu_widget'));
	} else {
	...
	if ($result) {
		$response['status'] = 'ok';
		$response['html'] = DorVerticalmenuWidget::renderAdminListWidgets();
	...
	$obj = new DorVerticalmenuWidget((int)Tools::getValue('id'));
	$result = false;
	if (Validate::isLoadedObject($obj)) {
		if ($obj->delete()) {
			$result = true;
	...
	$id = (int)Tools::getValue('id_dorverticalmenu');
	$obj = new ObjectDorVerticalmenu($id);
	$result = true;
	if (Validate::isLoadedObject($obj)) {
		$mscript = new DorVerticalmenuMcrypt();
		$data = $mscript->encode(serialize(Tools::getAllValues()));
		$obj->params = $data;
	//d(json_decode(Tools::getValue('params')));
