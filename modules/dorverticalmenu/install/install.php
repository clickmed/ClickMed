...
$res = (bool)Db::getInstance()->execute('
	CREATE TABLE `'._DB_PREFIX_.'dorverticalmenu` (
...
');
$res &= (bool)Db::getInstance()->execute('
	CREATE TABLE `'._DB_PREFIX_.'dorverticalmenu_lang` (
...
$res &= (bool)Db::getInstance()->execute('
	CREATE TABLE `'._DB_PREFIX_.'dorverticalmenu_shop` (
	  `id_dorverticalmenu` int(11) NOT NULL DEFAULT \'0\',
...
$res &= (bool)Db::getInstance()->execute('
	CREATE TABLE `'._DB_PREFIX_.'dorverticalmenu_widget` (
...
/* install sample data */
$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT id_dorverticalmenu FROM `'._DB_PREFIX_.'dorverticalmenu`');
if ( count($rows) == 0 ) {
	$res &=  (bool)Db::getInstance()->execute('
