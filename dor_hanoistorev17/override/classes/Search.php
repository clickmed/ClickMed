...
class SearchCore
{
        ...
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        ...
        if(isset($ajaxSearch) && $ajaxSearch == 1){
            $resultData = self::dorFindSearch($id_lang, $expr, $page_number, $page_size, $order_by, $order_way, $ajax, $use_cookie, $context);
    ...
    public static function dorFindSearch($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position',
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        /******Dorado Custom******/
        $cateID = Tools::getValue('cateID');
        $whereMore = "";
        if($cateID > 0){
            $whereMore = " AND cp.`id_category`=".$cateID;
        }
        if (!$context) {
            $context = Context::getContext();
        }
        ...
        AND product_shop.`visibility` IN ("both", "search")
        AND product_shop.indexed = 1
        '.$whereMore.$sql_groups, true, false);
        ...
        if (!$result) {
            ...
            foreach ($result_properties as $rawProduct) {
        ...
        return array('total' => $total,'result' => $products_for_template);
    }
    /**
     * @param Db $db
