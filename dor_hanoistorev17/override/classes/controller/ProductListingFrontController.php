...
abstract class ProductListingFrontControllerCore extends ProductPresentingFrontController
{
    ...
    protected function getAjaxProductSearchVariables()
    {
        ...
        if(isset($search['products']) && count($search['products']) > 0){
            ...
            foreach ($search['products'] as $rawProduct) {
            ...
            $search['products'] = $products_for_template;
        }
        
        $rendered_products_top = $this->render('catalog/_partials/products-top', array('listing' => $search));
        $rendered_products = $this->render('catalog/_partials/products', array('listing' => $search, 'carturl' => $this->context->link->getPageLink('cart', true),'static_token' => Tools::getToken(false)));
    ...
    protected function doProductSearch($template, $params = array(), $locale = null)
    {
        if ($this->ajax) {
            ...
            $products_for_template = [];
            if(isset($variables['products']) && count($variables['products']) > 0){
                $assembler = new ProductAssembler($this->context);
                ...
                foreach ($variables['products'] as $rawProduct) {
                    $products_for_template[] = $presenter->present(
                        $presentationSettings,
                        $assembler->assembleProduct($rawProduct),
                        $this->context->language
                ...
                $variables['products'] = $products_for_template;
            }
