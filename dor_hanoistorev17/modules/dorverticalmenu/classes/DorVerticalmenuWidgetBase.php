...
abstract class DorVerticalmenuWidgetBase {
	...
	public function getFormHelper()
	{
		...
		$this->input_fields = array(
			array(
				'type' => 'hidden',
				'label' => $this->l('ID'),
				'name' => 'id_dorverticalmenu_widget',
				'default' => 0,
			),
			array(
				'type' => 'hidden',
	...
	public function getConfigFieldsValues($data = null)
	{
			...
			foreach ($f['form']['input'] as $j => $input)
			{
				...
				else
				{
					if (isset($obj[trim($input['name'])]))
					{
						...
						{
							$thumb = _DORVERTICALMENU_IMAGE_URL_.$value;
		...
		if (isset($data['id_dorverticalmenu_widget'])) {
			$fields_values['id_dorverticalmenu_widget'] = $data['id_dorverticalmenu_widget'];
		}
