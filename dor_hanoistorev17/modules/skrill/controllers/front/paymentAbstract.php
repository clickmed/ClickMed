...
class SkrillPaymentAbstractModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        $this->saveInitialTransactionLog($postParameters);
    ...
    protected function saveInitialTransactionLog($postParameters)
    {
        ...
        VALUES "."('".
            $cartId."','".
            $postParameters['transaction_id']."','".
            $this->paymentMethod."','".
            $postParameters['currency']."','".
            $postParameters['amount'].
        "')";

        PrestaShopLogger::addLog('Skrill - save initial transaction log : ' . $sql, 1, null, 'Cart', $cartId, true);

        if (!Db::getInstance()->execute($sql)) {
            $logMessage = 'Skrill - failed when saving initial transaction log';
            PrestaShopLogger::addLog($logMessage, 3, null, 'Cart', $cartId, true);
            die('Erreur etc.');
        }
        PrestaShopLogger::addLog('Skrill - initial transaction log succefully saved', 1, null, 'Cart', $cartId, true);
    }
