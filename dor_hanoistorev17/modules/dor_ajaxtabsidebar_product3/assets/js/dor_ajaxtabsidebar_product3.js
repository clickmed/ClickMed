...
var SIDEBAR_PROCATEAJAXTAB3 = {
	init:function(){
		SIDEBAR_PROCATEAJAXTAB3.ActTab();
		//SIDEBAR_PROCATEAJAXTAB3.LoadMoreData();
	},
	ActTab:function(){
		jQuery(".sidebar3 #dorTabAjaxSidebar > li a").click(function(){
			var idTxt = jQuery(this).attr("href");
			jQuery("#dor-tabsidebar-product-category3 .tab-pane").removeClass("in active");
			jQuery(idTxt).addClass("in active");
			jQuery(".sidebar3 #dorTabAjaxSidebar > li a").removeClass("active");
			var objIdTxt = idTxt;
			...
			if(jQuery.trim(checkExist).length == 0 || checkExist == undefined){
				jQuery(".viewall-sidebar, #dorTabSidebarProductCategoryContent3 .view-all").hide();
				jQuery("#dorTabSidebarProductCategoryContent3").append('<span class="loaddingAjaxTab">Loadding...</span>');
				var urlAjax = jQuery(this).closest("ul").attr("data-ajaxurl");
				jQuery("#dorTabSidebarProductCategoryContent3 > div").css({
					'opacity':0,
					'min-height':400
				});
				var params = {}
				...
				jQuery.ajax({
		            ...
		            success:function(data){
		            	setTimeout(function(){
			            	...
			            	jQuery(".viewall-sidebar, #dorTabSidebarProductCategoryContent3 .view-all").show();
		            		SIDEBAR_PROCATEAJAXTAB3.SlideCarousel(".sidebar3 "+objIdTxt);
		            	},600);
		                return false;
		            }
		        });
...
jQuery(document).ready(function(){
	SIDEBAR_PROCATEAJAXTAB3.init();
	var idTxt = jQuery(".sidebar3 #dorTabAjaxSidebar li.active a").attr("href");
	SIDEBAR_PROCATEAJAXTAB3.SlideCarousel(idTxt);
});