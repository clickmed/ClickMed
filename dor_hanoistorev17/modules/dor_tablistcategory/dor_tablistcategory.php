...
class dor_tablistcategory extends Module {
    private $_html = '';
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->displayName = $this->l('Dor Tab List Category');
        $this->description = $this->l('Dor Tab List Category');
    ...
    public function install() {
        ...
        $tab->name[$language['id_lang']] = $this->l('Dor Tab List Category');
        $tab->class_name = 'AdminDorTabListCategory';
        $tab->id_parent = (int)Tab::getIdFromClassName('AdminDorMenu'); 
        $tab->module = $this->name;
        $tab->add();

        $today = date("Y-m-d");
        Configuration::updateValue($this->name . '_listtab_module_title', "");
        Configuration::updateValue($this->name . '_tablist_content_custom', "");
        Configuration::updateValue($this->name . '_show_all', 0);
        Configuration::updateValue($this->name . '_show_latest', 0);
        ...
        return parent::install() &&
                ...
                && 
                $this->registerHook('tablistcategory1');
    }

    public function uninstall() {
        $today = date("Y-m-d");
        $tab = new Tab((int) Tab::getIdFromClassName('AdminDorTabListCategory'));
        $tab->delete();
        Configuration::deleteByName($this->name . '_listtab_module_title', "");
        Configuration::deleteByName($this->name . '_tablist_content_custom', "");
        Configuration::deleteByName($this->name . '_show_all', 0);
        Configuration::deleteByName($this->name . '_show_latest', 0);
    ...
    private function _postProcess() {
        ...
        Configuration::updateValue($this->name . '_show_latest', Tools::getValue('show_latest'));
        ...
        Configuration::updateValue($this->name . '_cate_data', Tools::getValue('cate_data'));
    ...
    public function getConfigFieldsValues()
    {
        return array(
            ...
            'show_latest' =>     Tools::getValue('show_latest', Configuration::get($this->name . '_show_latest')),
    ...
    public function hookDisplayHeader() {
        ...
        //$this->context->controller->addCSS($this->_path . 'assets/css/dor_tablistcategory.css');
        $this->context->controller->addJS($this->_path . 'assets/js/dor_tablistcategory.js');
    }
    public function hookDisplayBackOfficeHeader()
    {
        $this->context->controller->addCSS($this->_path . 'assets/css/dor_tablistcategory_admin.css');
        $this->context->controller->addJS($this->_path . 'assets/js/dor_tablistcategory_admin.js');
    }
    public function hookHome(){
        ...
        if(Configuration::get($this->name . '_show_latest')) {
        ...
        if($defaultTabID != -1){
            $itemlists = $this->DorCacheTablist($cateID, $defaultTabID, $defaultTabID, $productTabslider);
            $products = $itemlists;
        }else{
            $productArray = array();
            if(Configuration::get($this->name . '_show_all')) {
        ...
        if($defaultTabID != -1){
            return $this->display(__FILE__, 'dor_tablistcategory.tpl', $this->getCacheId()."-".$defaultTabID."-".$id_shop."-".$id_lang);
        }else{
            return $this->display(__FILE__, 'dor_tablistcategory_all.tpl', $this->getCacheId()."-".$defaultTabID."-".$id_shop."-".$id_lang);
        }
    }

    public function DorCacheTablist($mainCateID, $cateID,$defaultTabID, $productTabslider){
        ...
        $id_shop = (int)Context::getContext()->shop->id;
        $id_lang = (int)Context::getContext()->language->id;
        $objCache = new DorCaches(array('name'=>'default','path'=>_PS_ROOT_DIR_.'/override/Dor/Caches/smartcaches/tablist1/','extension'=>'.cache'));
        $fileCache = 'HomeTabListCaches-Shop'.$id_shop."-".$id_lang.'-'.$cateID.'-'.$page;
        ...
        if($cacheData && $dorCaches && $page == 0){
            ...
            if(isset($_POST['cateID']) && $_POST['cateID'] != ""){
                if($_POST['type']==1){
                    $productData =  $this->getProductCategory ($cateID, $mergeCate, (int) Context::getContext()->language->id, $page, ($nb ? $nb : 10), null,  null);
    ...
    public function hooktablistcategory1($params){
        return $this->hookHome($params);
    }
    public function clearCache()
    {
        $this->_clearCache('dor_tablistcategory.tpl');
    }
    public function hookAjaxCall($params)
    {
        ...
        if(isset($_POST['cateID']) && $_POST['cateID'] != ""){
            ...
            $itemlists = $this->DorCacheTablist($cateID, $_POST['cateID'], $defaultTabID=0, $productTabslider=array());
