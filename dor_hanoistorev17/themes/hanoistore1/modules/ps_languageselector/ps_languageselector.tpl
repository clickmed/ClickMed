    ...
    <div class="language-selector">
      {foreach from=$languages key=k item=language name="languages"}
        {if $language.id_lang == $current_language.id_lang}
          <span class="txt-iso-code"><img alt="{$language.iso_code}" height="11" src="{$urls.img_lang_url}{$language.id_lang}.jpg" width="16" /><span class="code-name">{$current_language.name_simple|regex_replace:"/\s\(.*\)$/":""}</span></span>
        {/if}
      {/foreach}
