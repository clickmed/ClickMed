			...
			<div class="header-tablist">
				<div class="row">
					<div class="col-xs-12 col-sm-5 col-md-4 col-lg-3">
						<h3><i class="dor-icon-tab icon-tab-02"></i><span>{l s=$moduleTitle mod='dor_tablistcategory2'}</span></h3>
					</div>
			...
			<div class="protab-contents clearfix">
				<div class="row">
					<div class="slideImagesdata col-xs-12 col-sm-5 col-md-4 col-lg-3">
				    	{if $slideImages}
						<div class="ajax_block_product slideImagesList slide2">
				    ...
				    <div class="tab-content tablist-content-data col-xs-12 col-sm-7 col-md-8 col-lg-9" id="dorTabListCategoryContent2">
					{if $listTabsProduct}
						{$k=0}
						{foreach from=$listTabsProduct item=productTab name=tabCatePro}
						<div data-aria-labelledby="cate-tab-list2-{$productTab.id}-tab" id="cate-tab-list2-{$productTab.id}" class="tab-pane fade {if $productTab.id==$tabID.id} active {/if} in">
							...
							<a href="#" class="load-more-tab dor-icon-float-away tablists" data-page="2" data-limit="{$optionsConfig.listLimit2}" data-ajax="{if isset($urls.force_ssl) && $urls.force_ssl}{$urls.base_url_ssl}{else}{$urls.base_url}{/if}modules/dor_tablistcategory2/productcategory-ajax.php" onclick="return false">
					...
					{/if}
						...
						<div data-aria-labelledby="cate-tab-list2-{$cate.id}-tab" id="cate-tab-list2-{$cate.id}" class="tab-pane fade {if $cate.id==$tabID.id} active {/if} in">
							...
							<a href="#" class="load-more-tab dor-icon-float-away tablists" data-page="2" data-ajax="{if isset($urls.force_ssl) && $urls.force_ssl}{$urls.base_url_ssl}{else}{$urls.base_url}{/if}modules/dor_tablistcategory2/productcategory-ajax.php" data-limit="{$optionsConfig.listLimit2}" onclick="return false">
								<span class="clt-left"></span><span class="load-center"><i class="fa fa-plus"></i></span><span class="clt-right"></span>
							</a>
						</div>
						{$key= $key+1}
